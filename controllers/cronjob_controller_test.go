...
import (
	...
	cronjobv1 "github.com/zou2699/cronjob-tutorial/api/v1"
)
	...
	Context("When updating CronJob Status", func() {
		It("Should increase CronJob Status.Active count when new Jobs are created", func() {
			...
			/*
				...
				请注意，因为 k8s apiserver 在前面的 ‘Create()’ 调用之后可能还没有完成 CronJob 的创建，
				我将用 Gomega 的 Eventually() 测试函数代替 Expect() 去给 apiserver 一个机会去完成CronJob的创建。

				Eventually() 方法每隔一个时间间隔执行一次参数中指定的函数，直到满足下列两个条件之一才会退出方法。
				(a) 函数的输出与Should()调用的期望输出匹配 (b) 重试时间（重试次数 * 间隔周期）大于指定的超时时间
			...
			*/
			...
			/*
				...
				这确保了测试 Job 属于我们的测试 CronJob ，并被它跟踪。 一旦完成，我们就创建新的 Job 实例。
			*/
